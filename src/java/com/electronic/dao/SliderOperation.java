...
import com.electronic.beans.Slider;
...
public class SliderOperation extends DbConnection {
    ...
    public boolean addSlider(Slider slider) {
        ...
        try {
            ...
            pst = con.prepareStatement("insert into slides (image,title,subTitle,description,product_Id)"
                    + "Values (?,?,?,?,?)");
    ...
    public ArrayList<Slider> getAllSliders() {
        try {
            ...
            pst = con.prepareStatement("select * From slides");
             ...
             rs = pst.executeQuery();
            while (rs.next()) {
                Slider slide = new Slider(rs.getInt("id") ,rs.getString(5), rs.getString(2), rs.getString(3),
